...
class MockHLSServer {
    ...
    private func setupRoutes() {
        ...
        let universalHandler: ((HttpRequest) -> HttpResponse) = { [weak self] request in
            guard let self = self else { return .internalServerError }
            
            // Handle /normal/ prefixed requests (file serving)
            if request.path.hasPrefix("/normal/") {
            ...
            // Handle /not-found/ requests (always return 404)
            if request.path.hasPrefix("/not-found/") {
                return HttpResponse.notFound
            }
            
            // Handle general file serving from assets folder
        ...
        // Set the universal handler as the catch-all for unmatched requests
        server.notFoundHandler = universalHandler
