...
@property (nonatomic, strong) LocalHTTPServer *localServer;
...
- (AVPlayer *)testAVPlayer {
    ...
    BOOL shouldChangeVideo = [[NSProcessInfo.processInfo.environment objectForKey:@"SHOULD_CHANGE_VIDEO"] boolValue];
...
- (void)setupAVPlayerViewController:(AVPlayer *)player {
    ...
    if ([envKey isEqualToString:@"local"]) {
        self.localServer = [[LocalHTTPServer alloc] init];
    }
    if(envKey == nil) {
